...
/**
 * Next.js error boundary for /live route.
 * Catches and displays errors that occur during rendering or data fetching.
 * Includes auto-retry countdown functionality.
 */
export default function LiveError({
  ...
  useEffect(() => {
    if (!autoRetryEnabled || countdown <= 0) return;
  ...
  }, [countdown, autoRetryEnabled, reset]);
    ...
    <div className="max-w-7xl">
      <div className="rounded-xl border border-red-800/50 bg-red-900/10 p-12 text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-3">
          라이브 세션을 불러오는 중 오류가 발생했습니다
        </h2>
        <p className="text-sm text-gray-400 mb-6 max-w-md mx-auto">
          일시적인 문제일 수 있습니다. {countdown}초 후 자동으로 다시 시도합니다.
        </p>
        <div className="flex items-center justify-center gap-3">
          <button
            onClick={handleManualRetry}
            aria-label="수동으로 다시 시도"
